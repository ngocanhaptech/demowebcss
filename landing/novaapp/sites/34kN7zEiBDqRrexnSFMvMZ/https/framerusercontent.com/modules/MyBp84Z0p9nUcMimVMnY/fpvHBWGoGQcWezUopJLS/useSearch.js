...
} from "https://framerusercontent.com/modules/tV9haTHllpHHc9Fjue2H/bsLTiXYjkHoD2XaMjabu/SearchModal.js";
import {
    getCachedIndex,
    isDefaultLocaleId,
    setCachedIndex
    ...
    useEffect(() => {
        async function loadSearchIndex() {
            ...
            if (cacheResult.status === "fresh") {
                ...
                setStatus("success");
                log("Using fresh cached index");
                return;
            }
            if (cacheResult.status === "stale") {
                setSearchIndex(cacheResult.searchIndex);
                setStatus("loading-with-cache");
                log("Using stale cached index while loading a fresh one");
            }
            ...
            if (!fallbackBaseUrl) {
                ...
                return;
            } // If we're here, cache is missing or stale. If it's stale but it's our fallback index, we can use it.
            if (cacheResult.status === "stale" && cacheResult.indexHash === fallbackBaseUrl) {
                setStatus("success");
                log("Using cached fallback index");
                return;
            }
