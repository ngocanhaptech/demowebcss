// Generated by Framer (1bcc82d)
import {
    jsx as _jsx
} from "react/jsx-runtime";
import {
    cx,
    motion,
    useSVGTemplate,
    withCSS
} from "framer";
import * as React from "react";
import {
    forwardRef
} from "react";
const Base = /*#__PURE__*/ forwardRef(function(props, ref) {
    return /*#__PURE__*/ _jsx("svg", { ...props,
        ref: ref,
        children: props.children
    });
});
const MotionSVG = motion.create(Base);
const SVG = /*#__PURE__*/ forwardRef((props, ref) => {
    const {
        animated,
        layoutId,
        children,
        ...rest
    } = props;
    return animated ? /*#__PURE__*/ _jsx(MotionSVG, { ...rest,
        layoutId: layoutId,
        ref: ref,
        children: children
    }) : /*#__PURE__*/ _jsx("svg", { ...rest,
        ref: ref,
        children: children
    });
});
const svg = '<svg display="block" role="presentation" viewBox="0 0 112 36" xmlns="http://www.w3.org/2000/svg"><path d="M 15.838 23.67 L 3.366 5.176 L 3.366 23.67 L 0 23.67 L 0 0 L 3.96 0 L 16.135 18 L 16.135 0 L 19.501 0 L 19.501 23.67 Z M 30.69 24 C 25.74 24 22.111 20.11 22.111 14.934 C 22.111 9.758 25.74 5.868 30.69 5.868 C 35.639 5.868 39.269 9.758 39.269 14.934 C 39.269 20.11 35.639 24 30.69 24 Z M 30.69 21.066 C 33.56 21.066 35.837 18.758 35.837 14.934 C 35.837 11.11 33.56 8.835 30.69 8.835 C 27.819 8.835 25.575 11.11 25.575 14.934 C 25.575 18.758 27.819 21.066 30.69 21.066 Z M 55.647 6.198 L 49.08 23.67 L 45.451 23.67 L 38.884 6.198 L 42.481 6.198 L 47.332 19.846 L 52.248 6.198 Z M 67.228 23.67 L 67.228 20.967 C 66.14 22.912 64.193 24 61.619 24 C 57.989 24 55.614 21.923 55.614 18.725 C 55.614 15.297 58.352 13.517 63.566 13.517 C 64.588 13.517 65.413 13.549 66.833 13.714 L 66.833 12.429 C 66.833 9.923 65.479 8.505 63.17 8.505 C 60.728 8.505 59.243 9.956 59.144 12.396 L 56.109 12.396 C 56.273 8.472 59.078 5.868 63.17 5.868 C 67.492 5.868 70 8.308 70 12.462 L 70 23.67 Z M 58.781 18.626 C 58.781 20.44 60.134 21.626 62.279 21.626 C 65.084 21.626 66.833 19.879 66.833 17.209 L 66.833 15.791 C 65.546 15.626 64.622 15.593 63.763 15.593 C 60.431 15.593 58.781 16.582 58.781 18.626 Z" fill="var(--token-ded86de6-1b9d-4d9f-87d2-a30c6b424700, rgb(0, 0, 0))" height="24px" id="zYVrlFzKA" transform="translate(40.093 5.714)" width="70.0000008352115px"/><path d="M 23.397 9.77 C 21.832 9.003 20.073 8.571 18.214 8.571 C 11.705 8.572 6.429 13.849 6.429 20.357 C 6.429 21.045 6.492 21.717 6.604 22.372 C 2.693 20.454 0 16.435 0 11.785 C 0 5.276 5.277 0 11.786 0 C 17.608 0 22.44 4.221 23.397 9.77 Z M 30 20.357 C 30 26.866 24.723 32.143 18.214 32.143 C 12.392 32.142 7.561 27.921 6.604 22.372 C 8.168 23.139 9.927 23.571 11.786 23.571 C 18.295 23.571 23.571 18.294 23.571 11.785 C 23.571 11.098 23.509 10.425 23.397 9.77 C 27.307 11.687 30 15.708 30 20.357 Z" fill="rgb(27, 43, 184)" height="32.14260125px" id="vEJrACGPB" transform="translate(2 2)" width="30.00000028015137px"/></svg>';
const getProps = ({
    height,
    id,
    width,
    ...props
}) => {
    return { ...props
    };
};
const Component = /*#__PURE__*/ React.forwardRef(function(props, ref) {
    const {
        style,
        className,
        layoutId,
        variant,
        ...restProps
    } = getProps(props);
    const href = useSVGTemplate("2899812621", svg);
    return /*#__PURE__*/ _jsx(SVG, { ...restProps,
        className: cx("framer-MBypf", className),
        layoutId: layoutId,
        ref: ref,
        role: "presentation",
        style: { ...style
        },
        viewBox: "0 0 112 36",
        children: /*#__PURE__*/ _jsx("use", {
            href: href
        })
    });
});
const css = [".framer-MBypf { aspect-ratio: 3.111111111111111; display: block; width: 112px; }"];
/**
 * This is a generated Framer component.
 * @framerIntrinsicWidth 112
 * @framerIntrinsicHeight 36
 * @framerSupportedLayoutWidth any-prefer-fixed
 * @framerSupportedLayoutHeight any-prefer-fixed
 * @framerImmutableVariables true
 * @framerVector {"name":"Symbol+Text (Light)","set":{"localId":"vectorSet/wdss15BLE","id":"wdss15BLE","moduleId":"YfitM4ZQy6l0Ma6AVgsP"}}
 */
const Icon = withCSS(Component, css, "framer-MBypf");
Icon.displayName = "Symbol+Text (Light)";
export default Icon;
export const __FramerMetadata__ = {
    "exports": {
        "default": {
            "type": "reactComponent",
            "name": "Icon",
            "slots": [],
            "annotations": {
                "framerIntrinsicWidth": "112",
                "framerImmutableVariables": "true",
                "framerContractVersion": "1",
                "framerSupportedLayoutWidth": "any-prefer-fixed",
                "framerIntrinsicHeight": "36",
                "framerSupportedLayoutHeight": "any-prefer-fixed",
                "framerVector": "{\"name\":\"Symbol+Text (Light)\",\"set\":{\"localId\":\"vectorSet/wdss15BLE\",\"id\":\"wdss15BLE\",\"moduleId\":\"YfitM4ZQy6l0Ma6AVgsP\"}}"
            }
        },
        "__FramerMetadata__": {
            "type": "variable"
        }
    }
}